...
class BookSolutionDataSet {
  ...
  BookSolutionDataSet({ required this.booksolutionname, required this.booksolutionimagesrc, required this.topicDataset});
...
class TopicDataSet {
  ...
  TopicDataSet({required this.topicName, required this.subtopicDataSet});

  factory TopicDataSet.fromJson(Map<String, dynamic> data) {
    var list = data['subtopic_dataset'] as List;

    List<String> subtopicDataSetlist = new List<String>.from(list);
    return TopicDataSet(
        topicName: data['topic_name'], subtopicDataSet: subtopicDataSetlist);
  }
}
